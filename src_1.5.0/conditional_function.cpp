...
int calculate_Mvalue(vector<vector<vector<double> > > &z_score_, vector<vector<vector<double> > > &beta_,  vector<vector<vector<double> > > &sd_, int Index, string output, vector<vector<string> >  &variant_, map<string, bool> &pos, map<string, bool> &pos1, map<int,bool> &data_index, double z_cutoff,bool COV)
 {
   ...
   for(int i=0;i<cov_z_score.n_rows;i++)
    {
      ...
      for(int j=0;j<cov_z_score.n_cols;j++)
       {
//         cout<<i<<" "<<j<<endl;
         if(abs(cov_z_score(i,j))>abs(z_cutoff))
          {
             sym=1;
          }
       }
...
//change by Biao Zeng, 10:08 pm, 10/22/2021
//   mat Cov_1 =cov(cov_z_score_copy);
   
   mat Cov_1 =cor(cov_z_score_copy);
   if(COV)
    {
      Cov_1 =cov(cov_z_score_copy);
    }
...
void ConstructCov(vector<vector<vector<double> > > &z_score_, vector<vector<vector<double> > > &beta_,  vector<vector<vector<double> > > &sd_, int index, double z_cutoff, bool COV)
 {
...
//      cout<<cov_z_score_copy<<endl;
//      cout<<"Output data used to construct covariance is done"<<endl;  
  //Chnaged by Biao Zeng, 10:11 pm, 10/22/2021 
//   mat Cov =cov(cov_z_score_copy);
   mat Cov =cor(cov_z_score_copy);
   if(COV)
    {
      Cov =cov(cov_z_score_copy);
    }
...
//   cout<<"Infor2 is "<<z_score_[0][0].size()<<" "<<beta_[0][0].size()<<" "<<sd_[0][0].size()<<" "<<variant_[0].size()<<endl; 
   ...
   double P=ConstructCovRandom(z_score_, beta_, sd_ ,index , Res_fixed, Res_random, meta_mode, output1, variant_, pos, pos1, Res, MAF, VARIANCE, data_index, test1, test2,test3, ALLELE, cor_file, write_cor, Han,  wald, z_cutoff, COV);
//   cout<<"P is: "<<P<<endl;
   if(P<0)
    {
...
//   cout<<"Come to estimate Mvalue"<<endl;
   calculate_Mvalue(z_score_, beta_, sd_ ,index , output2,variant_, pos, pos1, data_index, z_cutoff, COV);  
    }
   return P;
 }
...
double MetaAnalysis(vector<vector<vector<double> > > &z_score_, vector<vector<vector<double> > >&beta_, vector<vector<vector<double> > >&sd_,int index, mat &Res_fixed, mat &Res_random, string meta_mode, string output, vector<vector<string> > &variant_, map<string, bool> &pos, int colocalization, map<string, string> &GWAS_file, map<string, double > &MAF, vector<double > &VARIANCE, map<int,bool> &data_index,string gene,vector<double> &test1_1, vector<double> &test2_1, vector<double> &test3_1, map<string, string> &ALLELE, string cor_file, bool write_cor, bool Han, bool wald, double z_cutoff, bool COV)
 {
//   cout<<"Come into MetaAnalysis function to perform final meta-analysis"<<endl;
   string output1= output+"_statistical_signal";
   map <string, bool> pos1;
...
//   cout<<"Perform real meta-analysis"<<endl;
   ...
   double P=ConstructCovRandom(z_score_, beta_, sd_ ,index , Res_fixed, Res_random, meta_mode, output1, variant_, pos, pos1, Res, MAF, VARIANCE, data_index,test1_1,test2_1, test3_1, ALLELE, cor_file, write_cor, Han, wald, z_cutoff,  COV);
      ...
      for(int I=0, indx=0;I<file_.size();)
       {
         ...
         if(grm_file_[I] != "NA")
           {
             if(readfam(file_[I])!=readfam(grm_file_[I])+1)
              {
                cout<<"incompatible number of individuals with phenotype and measured in grm matrix, please have a check on phenotype files and grm file "<<endl;
                exit(EXIT_FAILURE);
              }
           }
          ...
          if(covariate_[I] != "NA")
           {
             if(readfam(file_[I])!=readfam(covariate_[I])+1)
               {
                 cout<<"incompatible number of individuals with phenotype and covariate file, please have a check on phenotype files and grm file "<<endl;
                 exit(EXIT_FAILURE);
               }
          }
         ...
         conditional_index=conditional_analysis(file_[I], gene, totalCausalSNP, rho,histFlag,  gamma, weight, nthread, covariate_[I], grm_file_[I], OutputFile, geno_file_[I],z_score, beta, sd,1, variant, removal1, I, colocalization, Variance, wald);
         ...
         if(nested>0 && nested_1 >1 && file_.size()>nested)
          {
            ...
            string test_file=""; 
            MetaAnalysis(z_score_nested,  beta_nested, sd_nested, x, Res_fixed, Res_random, "fixed", Output1, variant_nested, pos, 0,GWAS_file, MAF, VARIANCE, data_index1, gene, test1_1, test2_1, test3_1, ALLELE, test_file,false, Han, wald, z_cutoff,  COV);
            ...
            bool INDEX=true;
            P=MetaAnalysis(z_score_nested2,  beta_nested2, sd_nested2, x, Res_fixed_2, Res_random_2, "random", Output2, variant_nested2, pos, 0,GWAS_file, MAF, VARIANCE2, data_index2, gene, INDEX, finemap, ALLELE,p_cutoff,cor_file,false, Han, wald, z_cutoff, COV);
          }
//          cout<<"Here3_2"<<endl; 
...
//          cout<<z_score_[0][0].size()<<endl;
          P1=MetaAnalysis(z_score_,  beta_, sd_, x, Res_fixed, Res_random, meta_mode, Output, variant_, pos, colocalization,GWAS_file, MAF, VARIANCE, data_index, gene, INDEX, finemap, ALLELE,p_cutoff,cor_file, write_cor, Han, wald, z_cutoff, COV);
          write_cor=false;
//          cout<<"Here3_3"<<endl;
          if(cis_summary)
           {
...
//Changed by Biao Zeng, 12/07/2021
...
int test1=int(floor(Z_score*Z_score*10.0));
int test2=int(ceil(Z_score*Z_score*10.0));

//double adjust1=Y1(nstudy-2, test1);

//double adjust2=Y1(nstudy-2,test2);
...
cout<<"original Z_score from random effect is: "<<Z_score<<endl;
cout<<"original pvalue from random effect is: "<<p<<endl;
...
if (test2 < 331) {
      ...
      cout<<"i is: "<<test1<<endl;

      cout<<"j is: "<<test2<<endl;
      ...
      double d9 = d7 / d8;
      cout<<"d9 is: "<<d9<<endl;
      double d10 = d6 + (d9 - d6) * (Z_score*Z_score - test1 / 10.0) / 0.1;
...
cout<<"Correct for covariance"<<endl;
for(int i=0; i<Cov1.n_rows; i++)
  {
    cout<<"I am here7"<<endl;
    for(int j=i+1; j<Cov1.n_rows; j++)
     {
       b1++;
       if(j==i)
        {
...
double D3 = D2 / b1;
cout<<"Mean covariance is: "<<D3<<endl;
...
if(Test2>=331)
 {
   Test2=330;
    ...
    double d4 = Y2(j,Test2) / Y2(0,Test2);
    cout<<"D4 is: "<<d4<<endl;
    p_final=p_final*d4;
   if(p_final>1.0)
    {
